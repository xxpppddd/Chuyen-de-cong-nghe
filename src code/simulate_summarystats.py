...
seq_gen_path = r"/mnt/d/Bio/Code_of_python/ModelRevelator/src/"
sim_params_path = r"/mnt/d/Bio/Code_of_python/ModelRevelator/simulation_parameters"
trees_df = pd.read_csv(os.path.join(sim_params_path, 'trees.csv.gz'), compression='gzip')
sim_target_dir = r"/mnt/d/Bio/Code_of_python/ModelRevelator/Result/Sim_output" # path for raw MSA files
train_data_path = r"/mnt/d/Bio/Code_of_python/ModelRevelator/Result/Train"
# test_data_path = os.getenv('TEST_DATA_DIR')
...
def process_msa_pairwise(lines):
    ...
    def make_profile(m):
        ...
        for c in range(0, num_positions):
            ...
            while True:
                first_strand = np.random.randint(0, m.shape[1], size=1, dtype=np.int64)[0]
                second_strand = np.random.randint(0, m.shape[1], size=1, dtype=np.int64)[0]
...
def process_msa_tstv(lines):
    ...
    idx = np.random.randint(0, msa.shape[1], size=num_positions, dtype=np.int64)
    msa = msa[:, idx]
...
def process_msa_conv(lines):
    ...
    for sample_count in range(samples):
        rand_taxa = np.random.randint(0, msa.shape[0], size=block_taxa, dtype=np.int64)
        rand_seq_start = np.random.randint(0, msa.shape[1], size=1, dtype=np.int64)[0]
    ...
    msa = np.zeros([len(sampled_msa), len(sampled_msa[0]), 4], dtype=np.int64)
...
def process_msa_frequencies(lines):
    ...
    idx = np.random.randint(0, msa.shape[1], size=freq_block_length, dtype=np.int64)
    msa = msa[:, idx]
...
def generate_msa(t):
    ...
    alpha = 0
    alpha_str = ''  # Khởi tạo alpha_str với giá trị mặc định rỗng

    if model[-2:] == '+G':
        alpha = potential_alphas  # Giả sử potential_alphas là một số hoặc giá trị được lấy ngẫu nhiên
        alpha_str = '_alpha{}'.format(alpha) 
    
    param_start, param_stop = mp[nr_taxa] * 10000, (mp[nr_taxa] + 1) * 10000
    random_numbers = [random.randint(param_start, param_stop) for _ in range(random_tree)]
    ...
    for idx in random_numbers:
        ...
        for i in range(random_times):
            ...
            fn_rand_salt = np.random.randint(0, 99999)
            ...
            path = os.path.join(sim_target_dir, str(length_) + 'bp', model)
            ...
            tmp_tree_file = filename + 'long_string.phy'
            with open(tmp_tree_file, 'w') as file:
                file.write(tree)
            cmd = (seq_gen_path + models[model]).format(**rates_freq, seq_len=seq_len, alpha=alpha) + ' < ' + tmp_tree_file + " > " + "\"" +full_path + "\""
            ...
            with subprocess.Popen([cmd],
                ...
                if len(lines) - 1 != nr_taxa:
                    ...
                    with subprocess.Popen(f"rm {tmp_tree_file}", shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE) as proc:
    ...
    #     idx = np.random.randint(param_start, param_stop, size=1, dtype=np.int64)[0]
    #     # print(idx)
    #     params = trees[idx]
    #     print(len(params))
    ...
    #     print(model, potential_alphas, seq_len, nr_taxa)
    #     start_time = time.time()
    #     rates_freq = {}
    #     for k, v in lanfear_eds.items():
    #         rates_freq.update({k: v.rvs(1)[0]})
    ...
    #     full_path = os.path.join(path, filename)
    #     cmd = (seq_gen_path + models[model]).format(**rates_freq, seq_len=seq_len, alpha=alpha) + ' <<< "{}"'.format(tree) + " > " + "\"" +full_path + "\""
...
def sim_msas(msas_per_model, models, num_threads, training_data=True, taxa=(8,),
    ...
    # print('Alphas per model:', alphas_per_model)
    ...
    # print(required_file_count)
    # print('Number of work packages:', len(work_packages))
    ...
    # print('Param sets per file:', param_sets_per_file)


    with ProcessPoolExecutor(max_workers=8) as executor:
        executor.map(generate_msa, work_packages)
    ...
    #         # test = 
    #         p.imap_unordered(generate_msa, sub, 5)
        ...
        # conv_data = np.zeros([len(msa_list), block_length, samples * block_taxa, 4], dtype=np.int64)
        # freq_data = np.zeros([len(msa_list), 10000, 4], dtype=np.float16)
        # labels = np.zeros([len(msa_list)], dtype=np.int64)
        ...
        #     conv_data[count, :, :] = conv_msa.astype(np.int64)
        ...
        # labels = np.eye(12, dtype=np.int64)[labels.reshape(-1)]
...
def main():
    ...
    sim_msas(20, ev_models, 10, save_msa=False, alphas=alphas, taxa=(10,    128), seq_len=length_, tfrecord_file_prefix='train_msas_1K', data_dir='./train_data_dir/', model_int_map=model_int_map)
